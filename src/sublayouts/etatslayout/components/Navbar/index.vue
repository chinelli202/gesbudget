  ...
  >
    ...
    >
      ...
      >
        ...
        <el-submenu
          v-for="(section, index) in depensesMap"
          :key="section.label"
          :index="1445-index"
        >
          <template slot="title">
          ...
          <div v-if="section.groupes && section.groupes.length > 0">
            <el-menu-item
              v-for="(element, index) in section.groupes"
              :key="154449-index"
              :label="element.label"
          ...
          <div v-else>
            <el-menu-item
              v-for="chapitre in section.chapitres"
              :key="chapitre.id"
              :index="chapitre.id"
...
private async paramNavTree() {
  ...
  const recettesOption = this.maquetteTree.recettes.chapitres

  if (this.domaine == 'fonctionnement') {
    const depensesOption = this.maquetteTree.depenses
    this.depensesMap = depensesOption.sections.map((section : any) => {
      const mappedSection = {
        label: section.section,
        value: section.section,
        groupes: section.groupes.map((groupe:any) => {
        // make custom names
          const urlname = groupe.label.split(' ').join('+')
          const sectiongroup = { label: groupe.label, value: urlname, type: 'groupe' }
          return sectiongroup
        }),
        chapitres: section.chapitres.map((chapitre:any) => {
          const mappedChapitre = { label: chapitre.label, value: chapitre.id, type: 'chapitre' }
    ...
    const depensesOption = this.maquetteTree.depenses.chapitres
    this.depensesMap = depensesOption.map((chapitre : any) => {
      const mappedChapitre = { label: chapitre.label, value: chapitre.id, index: chapitre.id, type: 'chapitre' }
      return mappedChapitre
    })
    console.log('this is the depense map, ', this.depensesMap)
  }
...
private handleMenuItemClicked(data:any) {
  ...
  var routename
  if (this.domaine == 'fonctionnement') { routename = 'element-fonctionnement' }
  if (this.domaine == 'mandat') { routename = 'element-mandat' }
...
private handleGenerauxClicked() {
  var routename
  if (this.domaine == 'fonctionnement') { routename = 'fonctionnement-generaux' }
  if (this.domaine == 'mandat') { routename = 'mandat-generaux' }
